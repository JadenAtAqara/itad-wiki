...
| Wake Signaling       | The complete process before UART communication: active-low assertion on the peer's Wake Pin, T0 wait, UART transfer, and pin release. See [Chapter 4](#chapter-4-initialization). |
| Link Establishment   | The first AECP interaction after power-on. Client sends `LinkEstablishmentRequest`; Server returns `LinkEstablishmentResponse(LowPowerFeature)`. See [Chapter 4](#chapter-4-initialization). |
| Profile Discovery    | The process in which Client READs Server Profiles and attribute initial values; Client sends `ProfileDiscoveryComplete` when done. See [Chapter 4](#chapter-4-initialization). |
| LowPowerFeature      | Single parameter in `LinkEstablishmentResponse`; negotiates Wake Signaling enablement; **enabled by default before negotiation**; `Normal` means negotiation disables it. See [Chapter 4](#chapter-4-initialization). |
...
| `Octets` / `Raw Data Octets`        | Number of Raw Data bytes for each parameter in COMMAND Payload Format (excluding the leading `Data Type` byte). Variable-length fields SHALL specify how length is calculated. |
| `Data Type`                         | Data type of each parameter in COMMAND Payload Format; on the wire it SHALL appear as a 1-byte `Data Type` prefix immediately before the corresponding Raw Data. |
| `Field Name`                        | Semantic name of a Raw Data column in the Wire Layout table. |
| `Wire Layout`                       | Complete Data Segment byte order for a COMMAND: table columns left to right are on-wire order, including Cmd ID and each `Data Type` / Raw Data column. |
...
- **wake pin (Wake Pin)**: Bidirectional wake-up signal for low-power devices (Wake Signaling uses this pin; see [Chapter 4](#chapter-4-initialization)). The Client and the Server each provide one wake pin so that each side can be woken by the other side.
...
Each parameter SHALL be encoded as `Data Type (1 B) + Raw Data (N B)`. Raw Data uses the same encoding as the Data field in a WRITE attribute block (without Attribute ID). Parameters SHALL be ordered by ascending `ID` in the `Parameter Details` table for this Command in the corresponding Profile. No parameter count field is carried.
...
- Parameter name, order, type, length, and constraints are defined by the **Wire Layout** in the Profile command definition.
...
1. **Link establishment**: Client sends `LinkEstablishmentRequest`; Server returns `LinkEstablishmentResponse(LowPowerFeature)` to negotiate whether Wake Signaling is enabled. **Before negotiation completes**, Wake Signaling is **enabled by default**; after negotiation, it is disabled only when Server returns `Normal`.
...
Link Establishment is the first AECP interaction process after power-on and is used to negotiate whether Wake Signaling is enabled (`LowPowerFeature`). Before negotiation completes, Wake Signaling is enabled by default.

**Completion criterion:** Client receives `LinkEstablishmentResponse` and caches `LowPowerFeature`.

**Interaction:**

- Client **SHALL** send `LinkEstablishmentRequest` (no parameters) as the first AECP frame after power-on; **SHALL** perform Wake Signaling before sending (enabled by default before negotiation).
- Server **SHALL** return `LinkEstablishmentResponse` carrying a single parameter `LowPowerFeature` (`LowPowerFeatureEnum8`, see [LowPowerFeatureEnum8](#lowpowerfeatureenum8)); **SHALL** perform Wake Signaling before sending.
...
| --- | --- | --- |
| Stage 1 | Link establishment | Negotiate `LowPowerFeature`; Wake Signaling enabled by default before negotiation; sender **SHALL** use Wake Signaling during negotiation; disabled when result is `Normal` |
...
1. Client **SHALL** perform Wake Signaling before sending `LinkEstablishmentRequest` (enabled by default before negotiation).
2. Server **SHALL** perform Wake Signaling before returning `LinkEstablishmentResponse(LowPowerFeature)`.
3. Client caches `LowPowerFeature`; link establishment negotiation completes. Wake Signaling is enabled by default before negotiation; disabled only when negotiation result is `Normal`.
...
* Client **SHALL** send `LinkEstablishmentRequest` as the first AECP interaction after power-on to negotiate `LowPowerFeature`; **SHALL** perform Wake Signaling before sending (enabled by default before negotiation).
* Server **SHALL** return `LinkEstablishmentResponse(LowPowerFeature)`; **SHALL** perform Wake Signaling before sending.
* Before both sides complete `LowPowerFeature` negotiation, Wake Signaling is enabled by default; during negotiation, the sender **SHALL** perform Wake Signaling.
...
| 0x0009 | ConnectionDisableRequest | Server → Client | ACKNOWLEDGE           | O           | Server requests Client to disable the wireless connection. |
| 0x000A | ProfileDiscoveryRequest        | Server → Client | ACKNOWLEDGE           | M           | Server requests Client to restart Profile Discovery. |
| 0x000B | LinkEstablishmentRequest    | Client → Server | LinkEstablishmentResponse | M       | Client initiates link establishment. | 
| 0x000C | LinkEstablishmentResponse   | Server → Client | N                     | M           | Server returns the low-power feature. |
...
For COMMAND frames with parameters, the **Data Segment** (payload after OPC) is encoded left to right as follows:

```
+--------+-----------+------+-----+-----------+------+
| Cmd ID | Data Type | Raw0 | ... | Data Type | RawN |
| (2 B)  | (1 B)     |(N0 B)|     | (1 B)     |(NN B)|
+--------+-----------+------+-----+-----------+------+
```
- **Cmd ID**: 2-byte big-endian unsigned integer; values are defined in the `ID` column of the command table above.
- **Parameter block**: each parameter consists of 1-byte `Data Type` followed by `Raw Data`; Raw Data encoding matches WRITE attribute Data (see [Data Types Overview](#data-types-overview)).
- **Parameter order**: for each command below, the **Wire Layout** table columns from left to right are the on-wire byte order; the first parameter block immediately follows Cmd ID.
- **D-Len**: total Data Segment length, equal to the sum of Wire Layout column lengths (see [COMMAND (OPC = 0x9)](#command-opc--0x9)). Parameter-less commands use `D-Len = 2`; parameterized commands use `D-Len = 2 + sum(1 + RawDataLen_i)`.
- **Wire Layout** shows the complete field names and lengths; **Field Descriptions** give semantics and constraints.

#### LinkEstablishmentRequest Commands

This command is sent by Server in response to `LinkEstablishmentRequest` to return the Server low-power feature. This command is a paired return command, and Client does not return ACKNOWLEDGE after receiving it.

#### LinkEstablishmentResponse Commands

This command is sent by Server in response to `LinkEstablishmentRequest` to return the Server low-power feature. This command is a paired return command, and Client does not return ACKNOWLEDGE after receiving it.

Command Payload Format (Wire Layout):

| Cmd ID | Data Type | LowPowerFeature |
| -- | -- | -- |
| 2 B | 1 B | 1 B |
| `0x000C` | `0x30` (enum8) | enum8 value |

Data Segment length: 4 B.

##### Field Descriptions

`LowPowerFeature` declares the Server low-power feature and negotiates whether Wake Signaling is enabled. Valid values are defined in [LowPowerFeatureEnum8](#lowpowerfeatureenum8). **Before both sides complete negotiation, Wake Signaling is enabled by default**; Server returning `Normal` means negotiation disables Wake Signaling (see [LowPowerFeature](#lowpowerfeature)).
...
#### ProfileDiscoveryComplete Commands
...
#### ProfileDiscoveryRequest Commands

This command is sent by Server to request Client to restart Profile Discovery (see [Chapter 4](#chapter-4-initialization)). Typical use cases include Server capability changes or updates to Profile or `ServerAttributeList` that require Client to re-read Server attributes and refresh its capability view.
...
#### ConnectionDisableRequest Commands

This command is sent by Server to request Client to disable the wireless connection. After accepting the request, Client SHALL disable RF transceiver functionality, disconnect from the wireless network, and return the execution result by ACKNOWLEDGE.

After disabling the wireless connection, Client SHOULD update `NetworkStatus` to `Offline` and synchronize it by NOTIFY when the state changes.
...
| Cmd ID | Data Type | Duration |
| -- | -- | -- |
| 2 B | 1 B | 2 B | 
| `0x0008` | `0x21` (uint16) | uint16 BE |

Data Segment length: 5 B.
...
| -- | -- | -- | -- | -- |
| 2 B | 1 B | 4 B | 1 B | 4 B |
| `0x0020` | `0x23` (uint32) | uint32 BE | `0x23` (uint32) | uint32 BE |

Data Segment length: 12 B.
...
Command Payload Format (Wire Layout):
| Cmd ID | Data Type | Offset | Data Type | Length |
| -- | -- | -- | -- | -- |
| 2 B | 1 B | 4 B | 1 B | 1 B |
| `0x0022` | `0x23` (uint32) | uint32 BE | `0x20` (uint8) | uint8 |

Data Segment length: 9 B.
...
| Cmd ID | Data Type | Offset | Data Type | Data (octstr) |
| -- | -- | -- | -- | -- |
| 2 B | 1 B | 4 B | 1 B | `1 + Length` B |
| `0x0023` | `0x23` (uint32) | uint32 BE | `0x41` (octstr) | length prefix + chunk bytes |

Data Segment length: `9 + Length` B (`Length` is the octstr chunk size, 1 ~ 255).
...
| Cmd ID | Data Type | Reason |
| -- | -- | -- |
| 2 B | 1 B | 1 B
| `0x0026` | `0x30` (enum8) | [OtaAbortReasonEnum8](#otaabortreasonenum8) value |
