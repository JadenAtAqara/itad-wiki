---
title: "Use Aqara Studio Skill in OpenClaw Web UI"
description: "This document explains how to use the Aqara Studio skill in the OpenClaw Web UI."
keywords: ["Aqara Docs", "Aqara Studio", "Use Aqara Studio Skill in OpenClaw Web UI"]
---
...
1. After installation, the Assistant will prompt you to provide `AQARA_ENDPOINT_URL` and `AQARA_OPEN_API_TOKEN` for authentication.
    ...
    :::tip
    - `AQARA_ENDPOINT_URL` is the HTTP Request URL of Aqara Studio, usually ending with `/open/api/v1`. The format is as follows:
        - Remote access disabled: `http://<Aqara_Studio_local_LAN_IP>/open/api/v1`
        - Remote access enabled: `https://<Aqara_Studio_remote_access_address>/open/api/v1`
    - `AQARA_OPEN_API_TOKEN` is the [token](./developer-guide.mdx#obtain-token) used for authentication in Aqara Studio.
    :::
