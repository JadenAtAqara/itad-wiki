...
基础组件包括 [text](#text)、[shape](#shape)、[line segment](#line-segment)、[button](#button)和[border](#border)。您可针对这些组件灵活设置**样式**属性，实现个性化的视觉效果。
...
- The **Action** block in Advanced Flow ([details](./../flow/device-blocks.mdx#action))
- The **Kit Batch Set** block in AutoLogic Flow ([details](./../logic/blocks/other-blocks.mdx#kit-batch-set))
