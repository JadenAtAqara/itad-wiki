---
title: "Aqara Empowerment Communication Protocol"
description: "This document defines the Aqara Empowerment Communication Protocol (AECP), a complete protocol stack for data exchange over UART between a wireless communication empowerment module and a host MCU."
keywords: ["Aqara Docs", "Aqara OpenLink", "Aqara Empowerment Communication Protocol"]
---
...
| Conformance | Implementation requirement marker. **M** (Mandatory) = the item SHALL be implemented; **O** (Optional) = the item MAY be implemented. `FeatureName` (no brackets) = SHALL be implemented when the corresponding FeatureMap bit is set; `[FeatureName]` (square brackets) = MAY be implemented when that bit is set. See [Field Notation Convention](#field-notation-convention). |
...
| `Conformance` / `Conformance` | Implementation requirement. `M` = Mandatory, SHALL be implemented; `O` = Optional, MAY be implemented. `FeatureName` (no brackets) = SHALL be implemented when FeatureMap bit `FeatureName` defined in the same Profile is set (mandatory under that Feature); `[FeatureName]` (square brackets) = MAY be implemented when that bit is set (optional under that Feature). For either Feature form, the item SHALL NOT be implemented when the corresponding bit is not set. `FeatureName` SHALL be the name of a FeatureMap bit defined in the same Profile. |
...
| 0x0008 | ModuleWirelessProtocol | Server retrieves wireless protocol of Client module firmware | 
...
The Conformance column describes implementation requirements: **M** (Mandatory, SHALL be implemented); **O** (Optional, MAY be implemented); **`FeatureName`** (no brackets, mandatory under that Feature); **`[FeatureName]`** (square brackets, optional under that Feature). Feature names and the requirement when the bit is not set follow [Field Notation Convention](#field-notation-convention).
...
#### ModuleWirelessProtocolEnum8

`ModuleWirelessProtocolEnum8` defines the wireless protocol of the firmware currently running on the Client module. The underlying type of this enumeration is enum8.

| Value | Name | Description |
| --- | --- | --- |
| 0x00  | Reserved | Reserved. |
| 0x01  | Matter   | The module is currently running Matter firmware. |
| 0x02  | Zigbee   | The module is currently running Zigbee firmware. |
...
| ID     | Name                              | Type                                                          | Constraint                | Default | Access | Conformance | Description                    |
| ------ | --------------------------------- | ------------------------------------------------------------- | ------------------------- | ------- | ------ | ----------- | ------------------------------ |
| 0x0000 | ProtocolVersion                   | uint16                                                        | Starts from 0             | 0x0000  | Q      | M           | AECP version                   |
| 0x0001 | ModuleFirmwareVersion             | uint32                                                        | No additional constraints | -       | Q      | M           | Client firmware version        |
| 0x0002 | ModuleUniqueID                    | octstr                                                        | Maximum 255               | -       | Q      | M           | Client unique identifier       |
| 0x0003 | PairingStatus                     | [PairingStatusEnum8](#pairingstatusenum8)                     | See enum                  | -       | Q/NF   | M           | pairing status                 |
| 0x0004 | NetworkStatus                     | [NetworkStatusEnum8](#networkstatusenum8)                     | See enum                  | -       | Q/NF   | M           | Client network status          |
| 0x0005 | LocalTime                         | uint32                                                        | No additional constraints | -       | Q/NF   | O           | Client local time              |
| 0x0006 | ModuleSupportedProductProfileList | list[uint16]                                                  | Non-reserved Profile ID   | -       | Q      | M           | supported product Profile list |
| 0x0007 | ClientUartBufferSize              | uint16                                                        | >= 1024 bytes             | 1024    | Q      | M           | Client UART buffer size        |
| 0x0008 | ModuleWirelessProtocol            | [ModuleWirelessProtocolEnum8](#modulewirelessprotocolenum8)   | See enum                  | -       | Q      | M           | module wireless protocol       |
...
#### ModuleWirelessProtocol

`ModuleWirelessProtocol` is maintained by Client and declares the wireless protocol of the firmware currently running on the module. This attribute has no default value. Valid values are defined in [ModuleWirelessProtocolEnum8](#modulewirelessprotocolenum8).

When Server needs to identify the Client wireless protocol, it SHALL retrieve this attribute by QUERY, and Client SHALL return the current value by QUERY_RESPONSE.

### Server Attributes

| ID     | Name                  | Type                                                         | Constraint                | Default | Access | Conformance | Description             |
|--------|-----------------------|--------------------------------------------------------------|---------------------------|---------|--------|-------------|-------------------------|
| 0x1000 | VendorID              | uint16                                                       | 1 ~ 0xFFFF                | 0x0000  | R      | M           | vendor ID               |
| 0x1001 | ProductID             | uint16                                                       | 1 ~ 0xFFFF                | 0x0000  | R      | M           | product ID              |
| 0x1002 | ServerHardwareVersion | uint16                                                       | 1 ~ 0xFFFF                | 0x0001  | R      | M           | Server hardware version |
| 0x1003 | ServerSoftwareVersion | uint16                                                       | 1 ~ 0xFFFF                | 0x0001  | R/RP   | M           | Server firmware version |
| 0x1004 | OtaUpgradeMethod      | [OtaUpgradeMethodEnum8](#otaupgrademethodenum8)              | See enum                  | 0x00    | R      | M           | OTA upgrade method      |
| 0x1005 | OtaStatus             | [OtaStatusEnum8](#otastatusenum8)                            | See enum                  | 0x00    | RP     | O           | OTA status              |
| 0x1006 | HardwareFault         | [HardwareFaultMap32](#hardwarefaultmap32)                    | See map                   | -       | R/RP   | O           | hardware fault bitmap   |
| 0x1007 | ProductProfileID      | uint16                                                       | Non-reserved Profile ID   | -       | R      | M           | product Profile         |
| 0x1008 | ServerUartBufferSize  | uint16                                                       | >= 1024 bytes             | 1024    | R      | M           | Server UART buffer size |
| 0xFFFB | ServerAttributeList   | list[uint16]                                                 | No additional constraints | -       | R      | M           | Server attribute list   |

#### VendorID

`VendorID` identifies the device manufacturer ID. Client SHALL READ this attribute during Profile Discovery. This attribute is a device-identity constant. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it.

#### ProductID

`ProductID` identifies the device product ID. Client SHALL READ this attribute during Profile Discovery. This attribute is a device-identity constant. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it.

#### ServerHardwareVersion

`ServerHardwareVersion` identifies the Server hardware version. Client SHALL READ this attribute during Profile Discovery. This attribute is a hardware-identity constant. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it.
...
Before initiating any OTA flow, Client SHALL first READ this attribute and select IAP OTA, BootLoader OTA, or abandon OTA according to the upgrade method returned by Server. This attribute is a capability constant. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it.
...
Client SHALL READ this attribute during Profile Discovery. This attribute is used only for capability discovery. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it. If the product Profile changes, the Server SHALL send `ProfileDiscoveryRequest` so that the Client READs it again.
...
Client SHALL READ this attribute during Profile Discovery. This attribute is used only for capability discovery. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it. If the supported attribute set changes, the Server SHALL send `ProfileDiscoveryRequest` so that the Client READs it again.
...
#### LocalConfigDisabledEnum8

`LocalConfigDisabledEnum8` defines whether local configuration is disabled. The underlying type is enum8.

| Value | Name | Description |
| --- | --- | --- |
| 0x00 | Enabled  | Local configuration is allowed. |
| 0x01 | Disabled | Local configuration is disabled. |
...
#### OpenWindowAlarmEnum8

The underlying type is enum8 and represents the open-window alarm state.

| Value | Name | Description |
| --- | --- | --- |
| 0x00 | NoAlarm | No open-window alarm. |
| 0x01 | InternalOpenWindowAlarm | Open window detected by the internal temperature probe. |
| 0x02 | ExternalOpenWindowAlarm | Open window detected by an external sensor. |

#### FrostProtectionAlarmEnum8

The underlying type is enum8 and represents the frost-protection alarm state.

| Value | Name | Description |
| --- | --- | --- |
| 0x00 | NoAlarm | No frost-protection alarm. |
| 0x01 | FrostProtectionAlarm | Frost protection is active. |

#### ThermostatFeatureMap32 

`ThermostatFeatureMap32` defines the thermostat feature bitmap. The underlying type is map32.

| Bit | Mask | Name | Description |
| --- | --- | --- | --- |
| 0 | 0x00000001 | Heating | The Server supports heating. |
| 1 | 0x00000002 | Cooling | The Server supports cooling. |

Feature conformance in this Profile's attribute and command tables uses the **Name** values in the table above.

#### AqaraThermostatFeatureMap32

`AqaraThermostatFeatureMap32` defines the Aqara Thermostat feature bitmap. The underlying type is map32.

| Bit | Mask | Name | Description |
| --- | --- | --- | --- |
| 0 | 0x00000001 | OpenWindowDetection | The Server supports open-window detection and alarm. |
| 1 | 0x00000002 | FrostProtection | The Server supports frost-protection alarm and a configurable trigger temperature. |
...
| 0x1001 | OccupiedHeatingSetpoint      | int16                            | See description  | -      | R/W/RP | S | Heating | heating setpoint
| 0x1002 | LocalTemperature             | int16                            | 0.01degC         | -      | R/RP   | S | M | ambient temperature
| 0x1003 | LocalTemperatureCalibration  | int8                             | 0.1degC          | 0      | R/W/RP | S | O | local temperature calibration
| 0x1004 | LocalConfigDisabled          | [LocalConfigDisabledEnum8](#localconfigdisabledenum8)  | See enum        | 0x00   | R/W/RP | S | O | local configuration disabled
...
| 0x1006 | FrostProtectionEnabled       | bool                             | true / false  | false  | R/W/RP | S | `[FrostProtection]` | frost-protection enable
| 0x1007 | FrostProtectionTemperature   | int16                            | Min ~ Max, 0.01degC | 500 | R/W/RP | S | FrostProtection | frost-protection trigger temperature
| 0x1008 | WindowDetectionEnabled       | bool                             | true / false  | false  | R/W/RP | S | OpenWindowDetection | open-window detection enable
| 0x1009 | ActiveCurrent                | uint32                           | mA            | -      | RP     | S | O | current
| 0x100A | ActivePower                  | uint32                           | mW            | -      | RP     | S | O | current power
| 0x100B | OpenWindowAlarm              | [OpenWindowAlarmEnum8](#openwindowalarmenum8) | See enum | 0x00 | R/RP | S | OpenWindowDetection | open-window alarm state
| 0x100C | OpenWindowTemperatureControlPauseDuration | uint16 | Min ~ Max, min | 30 | R/W/RP | S | `[OpenWindowDetection]` | temperature-control pause duration
| 0x100D | OpenWindowTemperatureControlPauseDurationMin | uint16 | min | 1 | R | S | `[OpenWindowDetection]` | minimum pause duration
| 0x100E | OpenWindowTemperatureControlPauseDurationMax | uint16 | min | 1440 | R | S | `[OpenWindowDetection]` | maximum pause duration
| 0x100F | FrostProtectionAlarm         | [FrostProtectionAlarmEnum8](#frostprotectionalarmenum8) | See enum | 0x00 | R/RP | S | FrostProtection | frost-protection alarm state
| 0x1010 | FrostProtectionTemperatureMin | int16 | 0.01degC | 500 | R | S | FrostProtection | minimum frost-protection temperature
| 0x1011 | FrostProtectionTemperatureMax | int16 | 0.01degC | 1500 | R | S | FrostProtection | maximum frost-protection temperature
| 0x1012 | CumulativeEnergy             | int64                            | mWh           | -      | RP     | S | O | cumulative imported energy
| 0x1014 | ThermostatFeatureMap         | [ThermostatFeatureMap32](#thermostatfeaturemap32)    | Heating=0x1, Cooling=0x2 | 0x00000001 | R     | S | M | thermostat feature bitmap
| 0x1015 | AbsMinHeatSetpointLimit      | int16                            | 0.01degC      | 500    | R      | S | `[Heating]` | absolute minimum heating setpoint
| 0x1016 | AbsMaxHeatSetpointLimit      | int16                            | 0.01degC      | 3000   | R      | S | `[Heating]` | absolute maximum heating setpoint
| 0x1017 | MinHeatSetpointLimit         | int16                            | 0.01degC      | 500    | R/W/RP | S | `[Heating]` | user minimum heating setpoint
| 0x1018 | MaxHeatSetpointLimit         | int16                            | 0.01degC      | 3000   | R/W/RP | S | `[Heating]` | user maximum heating setpoint
| 0x1019 | AqaraThermostatFeatureMap    | [AqaraThermostatFeatureMap32](#aqarathermostatfeaturemap32) | OpenWindowDetection=0x1, FrostProtection=0x2 | 0x00000003 | R | S | O | Aqara thermostat feature bitmap
| 0xFFFB | ServerAttributeList          | list[uint16]                    | No additional constraints    | - | R    | S | M | Server attribute list
...
`OccupiedHeatingSetpoint` represents the occupied heating target temperature, in units of 0.01degC. For example, `2650` means 26.50degC, and `-900` means -9.00degC. When the Heating bit of `ThermostatFeatureMap` is set, the Server SHALL implement this attribute. When that bit is not set, the Server SHALL NOT implement this attribute.

When this attribute is implemented, its value is constrained by the implemented heating limits. Unimplemented absolute limits SHALL be replaced by the table default values `500` / `3000`. Unimplemented user limits SHALL be replaced by the corresponding absolute limits, or by those defaults if the absolute limits are also unimplemented.

* `AbsMinHeatSetpointLimit` / `AbsMaxHeatSetpointLimit` are the manufacturer-declared absolute heating setpoint limits.
* `MinHeatSetpointLimit` / `MaxHeatSetpointLimit` are the user-configurable heating setpoint limits.
* When both the user limits and the absolute limits are implemented, they SHALL satisfy:
+
`AbsMinHeatSetpointLimit` ≤ `MinHeatSetpointLimit` ≤ `MaxHeatSetpointLimit` ≤ `AbsMaxHeatSetpointLimit`
* `OccupiedHeatingSetpoint` SHALL satisfy effectiveMin ≤ `OccupiedHeatingSetpoint` ≤ effectiveMax, where effectiveMin is the implemented `MinHeatSetpointLimit`, else the implemented `AbsMinHeatSetpointLimit`, else the default `500`; and effectiveMax is the implemented `MaxHeatSetpointLimit`, else the implemented `AbsMaxHeatSetpointLimit`, else the default `3000`.

When all limits are implemented, the complete relationship is:

`AbsMinHeatSetpointLimit` ≤ `MinHeatSetpointLimit` ≤ `OccupiedHeatingSetpoint` ≤ `MaxHeatSetpointLimit` ≤ `AbsMaxHeatSetpointLimit`

When this attribute is implemented, the Server SHALL REPORT it after the target temperature changes.
...
`LocalTemperatureCalibration` represents the local temperature calibration offset, in units of 0.1degC. For example, `20` means +2.0degC. The Server MAY implement this attribute.

When this attribute is implemented, the final local temperature = `LocalTemperature` + `LocalTemperatureCalibration`, and the Client MAY WRITE this attribute to adjust the calibration offset. If this attribute is not implemented, no calibration offset SHALL be applied, which is equivalent to a calibration value of `0`.

#### LocalConfigDisabled

`LocalConfigDisabled` indicates whether local configuration is disabled. Client MAY WRITE this attribute.

Valid values are defined in [LocalConfigDisabledEnum8](#localconfigdisabledenum8).
...
`FrostProtectionEnabled` indicates whether frost protection is enabled. `true` means enabled, and `false` means disabled.

When this attribute is `true`, the Server SHALL monitor the measured temperature used for frost protection against `FrostProtectionTemperature`. When this attribute is `false`, the Server SHALL NOT apply frost-protection behavior and MAY set `FrostProtectionAlarm` to `NoAlarm`. If this attribute is not implemented, whether frost protection is enabled is product-defined.

#### FrostProtectionTemperature

`FrostProtectionTemperature` represents the frost-protection trigger temperature, in units of 0.01degC. Its value SHALL be greater than or equal to `FrostProtectionTemperatureMin` and less than or equal to `FrostProtectionTemperatureMax`.

When `FrostProtectionEnabled` exists and is `true`, and the measured temperature used for frost protection is below this value, the Server SHALL trigger product-defined frost-protection behavior and set `FrostProtectionAlarm` to `FrostProtectionAlarm`. If `FrostProtectionEnabled` is absent, frost-protection triggering is product-defined.

#### WindowDetectionEnabled

`WindowDetectionEnabled` indicates whether open-window detection is enabled. `true` means enabled, and `false` means disabled.

When this attribute is `true`, the Server MAY use rapid temperature changes, external sensor input, or other product-defined logic to detect an open window. When this attribute is `false`, the Server SHALL NOT apply temperature-control behavior specific to open-window detection and MAY set `OpenWindowAlarm` to `NoAlarm` and immediately resume temperature control.
...
`OpenWindowAlarm` indicates the current open-window alarm state. Valid values are defined in [OpenWindowAlarmEnum8](#openwindowalarmenum8).

When an open window is detected while `WindowDetectionEnabled` is `true`, the Server SHALL set this attribute to `InternalOpenWindowAlarm` or `ExternalOpenWindowAlarm` and enter the temperature-control pause. After the alarm is cleared, the Server SHALL set this attribute to `NoAlarm` and resume temperature control.

The Server SHALL REPORT this attribute when its value changes. After successful execution of `ClearOpenWindowAlarmReq`, the Server SHALL clear the alarm and immediately resume temperature control.

#### OpenWindowTemperatureControlPauseDuration

`OpenWindowTemperatureControlPauseDuration` indicates how long temperature control is paused after an open-window alarm is triggered, in minutes. Its value SHALL be greater than or equal to `OpenWindowTemperatureControlPauseDurationMin` and less than or equal to `OpenWindowTemperatureControlPauseDurationMax`.

“Temperature-control pause” means that the Server suspends active heating or cooling output used to maintain the current target temperature, such as stopping a valve, relay, compressor, or other temperature-control actuator. A temperature-control pause does not turn off the device and does not change `SystemMode`, target temperatures, or other user configuration. During the pause, the Server SHALL continue temperature measurement, attribute reads/writes and reporting, command processing, and product-defined safety protection. Safety-protection behavior SHALL take precedence over the temperature-control pause.

After an open-window alarm is triggered, the Server SHALL enter the temperature-control pause for the duration specified by this attribute. When the duration expires, the Server SHALL resume normal temperature control according to the current `SystemMode`, target temperature, and measured temperature. If the alarm is cleared before the timer expires, the Server SHALL end the pause immediately and reevaluate temperature control.

#### OpenWindowTemperatureControlPauseDurationMin

`OpenWindowTemperatureControlPauseDurationMin` indicates the minimum value supported for `OpenWindowTemperatureControlPauseDuration`, in minutes. Server declares this as a device capability. Client SHALL READ it after Profile Discovery. This attribute is used only for capability declaration, and the Server SHALL NOT REPORT it.

#### OpenWindowTemperatureControlPauseDurationMax

`OpenWindowTemperatureControlPauseDurationMax` indicates the maximum value supported for `OpenWindowTemperatureControlPauseDuration`, in minutes. Server declares this as a device capability. Client SHALL READ it after Profile Discovery. This attribute is used only for capability declaration, and the Server SHALL NOT REPORT it.

#### FrostProtectionAlarm

`FrostProtectionAlarm` indicates the current frost-protection alarm state. Valid values are defined in [FrostProtectionAlarmEnum8](#frostprotectionalarmenum8). While frost protection is active, the Server SHALL set this attribute to `FrostProtectionAlarm`. After frost protection ends, the Server SHALL set it to `NoAlarm`.

The Server SHALL REPORT this attribute when its value changes.

#### FrostProtectionTemperatureMin

`FrostProtectionTemperatureMin` indicates the minimum value supported for `FrostProtectionTemperature`, in units of 0.01degC. Server declares this as a device capability. Client SHALL READ it after Profile Discovery. This attribute is used only for capability declaration, and the Server SHALL NOT REPORT it.

#### FrostProtectionTemperatureMax

`FrostProtectionTemperatureMax` indicates the maximum value supported for `FrostProtectionTemperature`, in units of 0.01degC. Server declares this as a device capability. Client SHALL READ it after Profile Discovery. This attribute is used only for capability declaration, and the Server SHALL NOT REPORT it.

#### CumulativeEnergy

`CumulativeEnergy` represents the cumulative imported energy, in mWh. The underlying type is int64. The value SHALL be non-decreasing. Server SHOULD REPORT this attribute according to the product-defined period or change policy.

#### ThermostatFeatureMap

`ThermostatFeatureMap` is the thermostat feature bitmap. Valid values are defined in [ThermostatFeatureMap32](#thermostatfeaturemap32). Server declares the value.

Client SHALL READ this attribute after Profile Discovery. This attribute is used only for capability declaration. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it.

#### AbsMinHeatSetpointLimit

`AbsMinHeatSetpointLimit` is the absolute minimum heating setpoint, in units of 0.01degC. Server declares this attribute.

If this attribute is present in `ServerAttributeList`, the Client SHALL READ it after Profile Discovery. This attribute is used only for capability declaration. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it. See [OccupiedHeatingSetpoint](#occupiedheatingsetpoint) for its relationship with `OccupiedHeatingSetpoint` and the other heating limits.

#### AbsMaxHeatSetpointLimit

`AbsMaxHeatSetpointLimit` is the absolute maximum heating setpoint, in units of 0.01degC. Server declares this attribute.

If this attribute is present in `ServerAttributeList`, the Client SHALL READ it after Profile Discovery. This attribute is used only for capability declaration. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it. See [OccupiedHeatingSetpoint](#occupiedheatingsetpoint) for its relationship with `OccupiedHeatingSetpoint` and the other heating limits.

#### MinHeatSetpointLimit

`MinHeatSetpointLimit` is the user-configurable minimum heating setpoint, in units of 0.01degC. If `AbsMinHeatSetpointLimit` is implemented, the value SHALL be greater than or equal to `AbsMinHeatSetpointLimit`. If `MaxHeatSetpointLimit` is implemented, the value SHALL be less than or equal to `MaxHeatSetpointLimit`. Client MAY WRITE this attribute. See [OccupiedHeatingSetpoint](#occupiedheatingsetpoint) for the complete relationship.

#### MaxHeatSetpointLimit

`MaxHeatSetpointLimit` is the user-configurable maximum heating setpoint, in units of 0.01degC. If `AbsMaxHeatSetpointLimit` is implemented, the value SHALL be less than or equal to `AbsMaxHeatSetpointLimit`. If `MinHeatSetpointLimit` is implemented, the value SHALL be greater than or equal to `MinHeatSetpointLimit`. Client MAY WRITE this attribute. See [OccupiedHeatingSetpoint](#occupiedheatingsetpoint) for the complete relationship.

`AqaraThermostatFeatureMap` is the Aqara Thermostat feature bitmap. Valid values are defined in [AqaraThermostatFeatureMap32](#aqarathermostatfeaturemap32). Server declares the value.

If this attribute is present in `ServerAttributeList`, the Client SHALL READ it after Profile Discovery. This attribute is used only for capability declaration. There is no runtime change-reporting scenario, and the Server SHALL NOT REPORT it.
...
| ID      | Name                      | Direction         | Response               | Conformance | Description                                                                      |
|---------|---------------------------|-------------------|------------------------|-------------|----------------------------------------------------------------------------------|
| 0x1000  | [ClearOpenWindowAlarmReq](#clearopenwindowalarmreq)   | Client → Server   | [ClearOpenWindowAlarmRsp](#clearopenwindowalarmrsp)| OpenWindowDetection           | Clear the active open-window alarm and immediately resume temperature control.    |
| 0x1001  | [ClearOpenWindowAlarmRsp](#clearopenwindowalarmrsp)   | Server → Client   | N                      | OpenWindowDetection           | Return the result of the clear operation.                                         |

Parameter definitions for commands with parameters are provided in each command's **Parameters** table. The Data Segment encoding is described in [COMMAND (OPC = 0x9)](#command-opc--0x9).

#### ClearOpenWindowAlarmReq

This command requests the Server to clear the active open-window alarm. After successful execution, the Server SHALL set `OpenWindowAlarm` to `NoAlarm` and immediately resume temperature control, regardless of any remaining `OpenWindowTemperatureControlPauseDuration`.

This command has no parameters. The Server SHALL return the result using the paired `ClearOpenWindowAlarmRsp` command and SHALL NOT return a separate ACKNOWLEDGE.

#### ClearOpenWindowAlarmRsp

This command is the paired response to `ClearOpenWindowAlarmReq`. The Client SHALL NOT send an ACKNOWLEDGE after receiving it.

##### Parameters

| Parameter Order | Name    | Type | Constraint     | Default |
|:---------------:|---------|------|---------------|---------|
| 1               | Success | bool | true/false    | -       |

`Success` indicates the result of the command: `true` means success and `false` means failure.
