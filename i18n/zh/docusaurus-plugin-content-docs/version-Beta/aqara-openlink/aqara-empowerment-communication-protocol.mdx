---
title: "Aqara 赋能通信协议规范"
description: "本文档定义 Aqara Empowerment Communication Protocol（中文名：Aqara 赋能通信协议），规范无线通信赋能模组与宿主 MCU 之间通过 UART 串口进行数据交换的完整协议栈。"
keywords: ["Aqara 文档", "Aqara OpenLink", "Aqara 赋能通信协议规范", "aqara empowerment communication protocol"]
---
# Aqara 赋能通信协议规范
...
- 上电初始化流程，包括 Link Establishment、Profile Discovery、Server 数据同步与 Wake Signaling（参见 [第四章](#第四章-初始化)）
...
Client 模组的 UART 接收缓冲区和发送缓冲区大小 SHALL 均不小于 **1024 字节**。双方 SHALL 在链路建立阶段交换 UART 缓冲区能力（参见 [Link Establishment](#link-establishment链路建立)），并取双方声明值中的**较小者**作为 **协商 UART 缓冲区大小**，用于约束后续 AECP 单帧长度。双方各自声明的值 SHALL NOT 小于 1024 字节，因此协商结果 SHALL NOT 小于 1024 字节。完整 AECP 帧长度从 `SOF` 开始计算，到 `Overall Checksum` 结束，包含帧头、`Data Segment` 和校验字段。
...
| 层次 | 职责 | 本规范对应 |
|:---|:---|:---|
| 物理层 | UART 电气与时序、Wake Pin 与 Wake Signaling | [第二章](#第二章-硬件接口)、[第四章](#第四章-初始化) |
...
| Conformance | 实现要求标注。**M**（Mandatory，必选）= 该条目 SHALL 实现；**O**（Optional，可选）= 该条目 MAY 实现。`FeatureName`（无括号）= 对应 FeatureMap 位置位时 SHALL 实现；`[FeatureName]`（方括号）= 对应 FeatureMap 位置位时 MAY 实现。详见 [字段记号约定](#字段记号约定)。 |
...
| Wake Signaling（唤醒信令） | 发送方在 UART 通信前，通过对端 Wake Pin 执行低电平断言、T0 等待、UART 传输与引脚释放的完整过程。参见 [Wake Signaling（唤醒信令）](#wake-signaling唤醒信令)。
| Link Establishment（链路建立） | 上电后首个 AECP 交互过程；Client 发送 `LinkEstablishmentRequest(ClientUartBufferSize)`，Server 返回 `LinkEstablishmentResponse(LowPowerFeature, ServerUartBufferSize)`。参见 [Link Establishment](#link-establishment)。 |
| Profile Discovery（Profile 发现） | Client READ Server Profile 与属性初始值的过程；完成后 Client 发送 `ProfileDiscoveryComplete`。参见 [Profile Discovery](#profile-discovery)。 |
| LowPowerFeature（低功耗特性） | `LinkEstablishmentResponse` 携带的参数，用于协商 Wake Signaling 是否使能；**双方协商完成前默认使能**；`Normal` 表示协商关闭。参见 [LowPowerFeature](#lowpowerfeature低功耗特性)。 | 
...
| `Conformance` / `必选性` | 实现要求。`M` = Mandatory，必选，SHALL 实现；`O` = Optional，可选，MAY 实现。`FeatureName`（无括号）= 同一 Profile 内已定义 FeatureMap 位 `FeatureName` 置位时 SHALL 实现（该 Feature 下必选）；`[FeatureName]`（方括号）= 该位置位时 MAY 实现（该 Feature 下可选）。两种 Feature 写法在对应位未置位时均 SHALL NOT 实现。`FeatureName` SHALL 为同一 Profile 内已定义 FeatureMap 位的名称。 |
...
| `Parameter Details` / `参数`        | 某条 COMMAND 的参数定义表：列出参数顺序、名称、类型、约束与默认值；参数 Raw Data 长度由类型决定（参见 [数据类型总览](#数据类型总览)）；编码规则见 [COMMAND（OPC = 0x9）](#commandopc--0x9)。 |
...
- **唤醒引脚（Wake Pin）**：用于低功耗设备的双向唤醒信号（Wake Signaling 使用该引脚，参见 [Wake Signaling（唤醒信令）](#wake-signaling唤醒信令)）。Client 和 Server 各自提供一个唤醒引脚，分别用于被对方唤醒。
...
其中 `10 bytes` 固定开销 = 帧头 8 字节（SOF ~ HCS）+ Overall Checksum 2 字节。协商 UART 缓冲区大小 SHALL 取 `min(ClientUartBufferSize, ServerUartBufferSize)`，双方各自声明的值 SHALL NOT 小于 1024 字节，协商结果 SHALL NOT 小于 1024 字节；协商规则见 [Link Establishment](#link-establishment)。UART 缓冲区要求见 [UART 缓冲区约束](#uart-缓冲区约束)。
...
- **有参数命令**：D-Len = `2 + Σ(1 + sizeof(raw_i))`。
- 参数名称、顺序、类型和约束由 Profile 命令定义中的 **Parameter Details**（参数）表决定；参数 Raw Data 长度由 [数据类型纵览](#数据类型总览) 中的类型定义决定。
...
1. **链路建立**：Client 发送 `LinkEstablishmentRequest(ClientUartBufferSize)`，Server 返回 `LinkEstablishmentResponse(LowPowerFeature, ServerUartBufferSize)`，协商 Wake Signaling 是否使能，并交换双方 UART 缓冲区能力；双方取较小值作为 AECP 单帧长度上限（>= 1024 字节）。双方协商完成前，Wake Signaling 特性**默认使能**；协商完成后，仅当 Server 返回 `Normal` 时不使能。
2. **Profile 发现**：Client READ Server Profile 与属性初始值，并发送 `ProfileDiscoveryComplete`；Server ACK 后 Profile 发现完成。
3. **Server 数据同步**：Server 按需 QUERY Client 属性或 REPORT Server 属性；初始化完成后进入正常工作。
...
- Client **SHALL** 发送 `LinkEstablishmentRequest(ClientUartBufferSize)` 作为上电首条 AECP 帧；`ClientUartBufferSize` 语义见 [ClientUartBufferSize 属性](#clientuartbuffersize-属性)，且 SHALL NOT 小于 1024 字节；发送前 **SHALL** 执行 Wake Signaling（协商前默认使能）。
- Server **SHALL** 返回 `LinkEstablishmentResponse(LowPowerFeature, ServerUartBufferSize)`；`ServerUartBufferSize` 语义见 [ServerUartBufferSize 属性](#serveruartbuffersize-属性)，且 SHALL NOT 小于 1024 字节；发送前 **SHALL** 执行 Wake Signaling。
...
协商结果 SHALL NOT 小于 1024 字节。后续任意方向的 AECP 单帧，完整帧长度（见 [交互模式](#交互模式)）SHALL NOT 超过协商 UART 缓冲区大小。
...
`LowPowerFeature` 是 `LinkEstablishmentResponse` 携带的参数，类型为 [LowPowerFeatureEnum8](#lowpowerfeatureenum8-类型)，用于协商 Wake Signaling 是否使能。**双方协商完成前，Wake Signaling 特性默认使能**；Server 返回 `Normal` 表示协商关闭 Wake Signaling。
...
- **协商前默认**：双方协商 `LowPowerFeature` 完成前，Wake Signaling 特性默认使能。
- **链路建立协商期间**：因协商前默认使能，发送方 **SHALL** 执行 Wake Signaling。
- **协商结果为 `Normal`**：Wake Signaling 不使能，双方直接 UART 通信。
- **协商结果为 `LowPower`（默认）**：Wake Signaling 保持使能；每次 UART 通信前，发送方 **SHALL** 执行 Wake Signaling（Client→Server、Server→Client 均适用）。

![Wake Signaling 时序图](https://itad-aqara-docs-1300889962.aqara.com/dev/img/mcu-standard-protocol/zh/wake_timing.svg)
...
Client 上电后 **SHALL** 首先执行链路建立（参见 [Link Establishment（链路建立）](#link-establishment链路建立)）：

1. Client 发送 `LinkEstablishmentRequest(ClientUartBufferSize)` 前 **SHALL** 执行 Wake Signaling（协商前默认使能）。
2. Server 返回 `LinkEstablishmentResponse(LowPowerFeature, ServerUartBufferSize)` 前 **SHOULD** 执行 Wake Signaling。
3. Client 缓存 `LowPowerFeature`、`ServerUartBufferSize` 与协商 UART 缓冲区大小；Server 缓存 `ClientUartBufferSize` 与协商 UART 缓冲区大小；链路建立协商完成。协商前 Wake Signaling 默认使能；仅当协商结果为 `Normal` 时不使能。

#### Profile 发现

Client **SHALL** 在链路建立完成后执行 Profile 发现，按推荐 READ 顺序读取 Server 能力视图与属性初始值，随后发送 `ProfileDiscoveryComplete`。

- `ProfileDiscoveryRequest` 触发时，Client **SHALL** 重新执行 Profile 发现（**SHALL NOT** 重做链路建立）。
- Client **SHALL** 使用 `Profile ID = 0x0000` 发送 `ProfileDiscoveryComplete`。
- Server 返回 `ACKNOWLEDGE(Status=0x00)` 后，Profile 发现完成。
...
| 0x0008 | ModuleWirelessProtocol | Server 拉取 Client 模组当前无线协议 |
...
![初始化启动流程](https://itad-aqara-docs-1300889962.aqara.com/dev/img/mcu-standard-protocol/zh/initialization_sequence.svg)
...
Conformance 列说明实现要求：**M**（Mandatory，必选，SHALL 实现）；**O**（Optional，可选，MAY 实现）；**`FeatureName`**（无括号，该 Feature 下必选）；**`[FeatureName]`**（方括号，该 Feature 下可选）。Feature 名称与未置位时的要求见 [字段记号约定](#字段记号约定)。
...
上电初始化 SHALL 由 Client 主导，按 Link Establishment → Profile Discovery → Server 数据同步 顺序执行（详细流程参见 [第四章](#第四章-初始化)）。

#### Link Establishment

* Client **SHALL** 发送 `LinkEstablishmentRequest(ClientUartBufferSize)` 作为上电首条 AECP 交互，协商 `LowPowerFeature` 并声明 Client UART 缓冲区能力；发送前 **SHALL** 执行 Wake Signaling（协商前默认使能）。
* Server **SHALL** 返回 `LinkEstablishmentResponse(LowPowerFeature, ServerUartBufferSize)`；发送前 **SHALL** 执行 Wake Signaling。
* 链路建立完成后，双方 SHALL 计算协商 UART 缓冲区大小 `min(ClientUartBufferSize, ServerUartBufferSize)` 并用于约束后续单帧长度；双方声明值 SHALL NOT 小于 1024 字节。
* 双方协商 `LowPowerFeature` 完成前，Wake Signaling 特性默认使能；协商交互期间，发送方 **SHALL** 执行 Wake Signaling。
...
* 当 `LowPowerFeature=LowPower` 时，Client **SHALL** 在每次 Client→Server READ 前执行 Wake Signaling。
* Client 完成 Profile Discovery 后 **SHALL** 发送 `ProfileDiscoveryComplete`。
...
#### ModuleWirelessProtocolEnum8 类型

`ModuleWirelessProtocolEnum8` 定义 Client 模组当前运行固件所属的无线协议。该枚举的底层类型为 enum8。

| 值    | 名称     | 说明 |
|:---:|:---|:---|
| 0x00  | Reserved | 预留。 |
| 0x01  | Matter   | 模组当前运行 Matter 固件。 |
| 0x02  | Zigbee   | 模组当前运行 Zigbee 固件。 |
...
`LowPowerFeatureEnum8` 定义 Server 低功耗特性。该枚举的底层类型为 enum8。Client 通过 `LinkEstablishmentResponse` 获取该值（参见 [第四章](#第四章-初始化)），用于协商 Wake Signaling 是否使能。**双方协商完成前，Wake Signaling 特性默认使能**。
...
| ID | 名称 | 类型 | 约束 | 默认值 | 访问权限 | 必选性 | 简述 |
|:---|:---|:---|:---|:---|:---|:---|:---|
| 0x0000 | ProtocolVersion | uint16 | 从 0 开始 | 0x0000 | Q | M | AECP 版本号 |
| 0x0001 | ModuleFirmwareVersion | uint32 | 无额外约束 | — | Q | M | Client 固件版本 |
| 0x0002 | ModuleUniqueID | octstr | 最长 255 | — | Q | M | Client 唯一标识 |
| 0x0003 | PairingStatus | [PairingStatusEnum8](#pairingstatusenum8-类型) | 见枚举 | — | Q/NF | M | 配网状态 |
| 0x0004 | NetworkStatus | [NetworkStatusEnum8](#networkstatusenum8-类型) | 见枚举 | — | Q/NF | M | Client 网络状态 |
| 0x0005 | LocalTime | uint32 | 无额外约束 | — | Q/NF | O | Client 本地时间 |
| 0x0006 | ModuleSupportedProductProfileList | list[uint16] | 非保留 Profile ID | — | Q | M | 支持的产品 Profile 列表 |
| 0x0007 | ClientUartBufferSize | uint16 | >= 1024 字节 | 1024 | Q | M | Client UART 缓冲区大小 |
| 0x0008 | ModuleWirelessProtocol | [ModuleWirelessProtocolEnum8](#modulewirelessprotocolenum8-类型) | 见枚举 | — | Q | M | 模组无线协议 |
...
#### ModuleWirelessProtocol 属性

`ModuleWirelessProtocol` 由 Client 维护，用于声明模组当前运行固件所属的无线协议。该属性无默认值。有效值见 [ModuleWirelessProtocolEnum8 类型](#modulewirelessprotocolenum8-类型)。

Server 需要识别 Client 无线协议时，SHALL 通过 QUERY 拉取该属性，Client SHALL 通过 QUERY_RESPONSE 返回当前值。

### Server 属性

| ID     | 名称                   | 类型                                                         | 约束              | 默认值   | 访问权限 | 必选性 | 简述                   |
|:-------|:-----------------------|:-------------------------------------------------------------|:------------------|:---------|:---------|:-------|:-----------------------|
| 0x1000 | VendorID               | uint16                                                       | 1 ~ 0xFFFF        | 0x0000   | R        | M      | 厂商 ID                |
| 0x1001 | ProductID              | uint16                                                       | 1 ~ 0xFFFF        | 0x0000   | R        | M      | 产品 ID                |
| 0x1002 | ServerHardwareVersion  | uint16                                                       | 1 ~ 0xFFFF        | 0x0001   | R        | M      | Server 硬件版本        |
| 0x1003 | ServerSoftwareVersion  | uint16                                                       | 1 ~ 0xFFFF        | 0x0001   | R/RP     | M      | Server 固件版本        |
| 0x1004 | OtaUpgradeMethod       | [OtaUpgradeMethodEnum8](#otaupgrademethodenum8-类型)         | 见枚举            | 0x00     | R        | M      | OTA 升级方式           |
| 0x1005 | OtaStatus              | [OtaStatusEnum8](#otastatusenum8-类型)                       | 见枚举            | 0x00     | RP       | O      | OTA 状态               |
| 0x1006 | HardwareFault          | [HardwareFaultMap32](#hardwarefaultmap32-类型)               | 见 map            | —        | R/RP     | O      | 硬件故障位图           |
| 0x1007 | ProductProfileID       | uint16                                                       | 非保留 Profile ID | —        | R        | M      | 产品 Profile           |
| 0x1008 | ServerUartBufferSize   | uint16                                                       | >= 1024 字节      | 1024     | R        | M      | Server UART 缓冲区大小 |
| 0xFFFB | ServerAttributeList    | list[uint16]                                                 | 无额外约束        | —        | R        | M      | Server 属性列表        |

#### VendorID 属性

`VendorID` 标识设备制造商 ID。Client SHALL 在 Profile Discovery 期间 READ 该属性。该属性为设备身份常量，不存在运行期变化上报场景，Server SHALL NOT REPORT。

#### ProductID 属性

`ProductID` 标识设备产品 ID。Client SHALL 在 Profile Discovery 期间 READ 该属性。该属性为设备身份常量，不存在运行期变化上报场景，Server SHALL NOT REPORT。

#### ServerHardwareVersion 属性

`ServerHardwareVersion` 标识 Server 硬件版本。Client SHALL 在 Profile Discovery 期间 READ 该属性。该属性为硬件身份常量，不存在运行期变化上报场景，Server SHALL NOT REPORT。
...
Client 发起任何 OTA 流程前 SHALL 先 READ 该属性，并按 Server 返回的升级方式选择 IAP OTA、BootLoader OTA 或放弃 OTA。该属性为能力常量，不存在运行期变化上报场景，Server SHALL NOT REPORT。
...
Client SHALL 在 Profile Discovery 期间 READ 该属性。该属性仅用于能力发现，不存在运行期变化上报场景，Server SHALL NOT REPORT。若产品 Profile 发生变化，Server SHALL 发送 `ProfileDiscoveryRequest`，由 Client 重新 READ。

#### ServerUartBufferSize 属性

`ServerUartBufferSize` 由 Server 维护，用于声明 Server UART 接收和发送缓冲区大小，单位字节。

该属性类型为 `uint16`，默认值为 `1024`。Server UART 接收缓冲区和发送缓冲区 SHALL 均不小于该值；若接收和发送缓冲区大小不同，该属性 SHALL 使用较小值作为可用单帧长度能力。链路建立时，Server SHALL 在 `LinkEstablishmentResponse` 中携带与该属性一致的 `ServerUartBufferSize` 参数。该属性为链路能力常量，不存在运行期变化上报场景，Server SHALL NOT REPORT。
...
Client SHALL 在 Profile Discovery 期间 READ 该属性。该属性仅用于能力发现，不存在运行期变化上报场景，Server SHALL NOT REPORT。若支持的属性集合发生变化，Server SHALL 发送 `ProfileDiscoveryRequest`，由 Client 重新 READ。
...
有参命令的参数定义见各命令 **参数** 表；Data Segment 编码规则见 [COMMAND（OPC = 0x9）](#commandopc--0x9)。

#### LinkEstablishmentRequest 命令

该命令由 Client 在上电后作为首条 AECP 交互发送，用于发起 Link Establishment（参见 [Link Establishment（链路建立）](#link-establishment链路建立)）。Client SHALL 在链路建立阶段声明 Client UART 缓冲区能力；双方协商后取较小值作为 AECP 单帧长度上限。发送前 Client **SHALL** 执行 Wake Signaling（协商前默认使能）。
...
](#clientuartbuffersize-属性)）一致。Client UART 接收缓冲区和发送缓冲区 SHALL 均不小于该值；若两者大小不同，该参数 SHALL 使用较小值。
...
| 参数顺序 | 名称 | 类型 | 约束 | 默认值
|:--------:|:--------:|:--------:|:--------:|:--------:|
| 1 | LowPowerFeature | [LowPowerFeatureEnum8](#lowpowerfeatureenum8-类型) | 见枚举 | —
| 2 | ServerUartBufferSize | uint16 | >= 1024 字节 | —

##### 字段说明

* `LowPowerFeature` 声明 Server 低功耗特性，用于协商 Wake Signaling 是否使能。有效值见 [LowPowerFeatureEnum8](#lowpowerfeatureenum8-类型)。**双方协商完成前，Wake Signaling 特性默认使能**；Server 返回 `Normal` 表示协商关闭 Wake Signaling（参见 [LowPowerFeature 属性](#lowpowerfeature低功耗特性)）。Server 报告的 `LowPowerFeature` **SHALL** 与产品实际低功耗能力一致。未实现低功耗 sleep 与 Wake Pin 机制的产品 **SHALL** 返回 `Normal`。
* `ServerUartBufferSize` 声明 Server UART 接收与发送缓冲区能力，单位字节，语义与同名 Server 属性（参见 [ServerUartBufferSize 属性](#serveruartbuffersize-属性)）一致。Server UART 接收缓冲区和发送缓冲区 SHALL 均不小于该值；若两者大小不同，该参数 SHALL 使用较小值。

#### ProfileDiscoveryComplete 命令

该命令由 Client 在完成 Profile Discovery（参见 [Profile Discovery（Profile 发现）](#profile-discoveryprofile-发现)）后发送，用于通知 Server：Client 侧 Profile Discovery 已经完成，Server 可以开始主动数据同步。
...
该命令由 Server 发送，用于请求 Client 重新执行 Profile Discovery（参见 [Profile Discovery（Profile 发现）](#profile-discoveryprofile-发现)）。典型场景包括 Server 能力变更、Profile 或 `ServerAttributeList` 更新后，需要 Client 重新读取 Server 属性并同步能力视图。
...
- `Enabled = true`：Client SHALL 打开射频收发功能并尝试接入无线网络。接入过程中 Client SHOULD 更新 `NetworkStatus` 为 `Connecting`；接入成功后 SHOULD 更新为 `Online`，并在状态变化时通过 NOTIFY 同步。
- `Enabled = false`：Client SHALL 关闭射频收发功能并断开无线网络连接。Client SHOULD 更新 `NetworkStatus` 为 `Offline`，并在状态变化时通过 NOTIFY 同步。
...
| 参数顺序 | 名称 | 类型 | 约束 | 默认值 |
|:--------:|:--------:|:--------:|:--------:|:--------:|
| 1 | Reason | [OtaAbortReasonEnum8](#otaabortreasonenum8-类型) | 见枚举 | — |
...
7. **重新初始化阶段**：Client OTA 完成并重启后，SHALL 重新执行 Link Establishment 与 Profile Discovery（参见 [第四章](#第四章-初始化)），读取 `ProductProfileID`、各 Profile 的 `ServerAttributeList` 以及已发现 Server 属性的当前值。Client 完成重新发现后，SHALL 再发送 `ProfileDiscoveryComplete`。Server 返回 `ACKNOWLEDGE(Status=0x00)` 前，SHALL NOT 主动 QUERY Client 属性或 REPORT Server 属性；ACK 完成后，Server MAY 重新开始主动数据同步，确保 Server 更新后的能力与 Client 更新后的协议栈能力重新同步。
...
#### LocalConfigDisabledEnum8 类型

`LocalConfigDisabledEnum8` 定义本地配置是否禁用。该枚举的底层类型为 enum8。

| 值 | 名称 | 说明 |
|:---:|:---|:---|
| 0x00 | Enabled  | 允许本地配置。|
| 0x01 | Disabled | 禁用本地配置。|
...
#### OpenWindowAlarmEnum8 类型

该类型的底层类型为 `enum8`，用于表示开窗告警状态。

| 值    | 名称                    | 说明                    |
|-------|-------------------------|-------------------------|
| 0x00  | NoAlarm                 | 无开窗告警。            |
| 0x01  | InternalOpenWindowAlarm | 内部温度探头检测到开窗。|
| 0x02  | ExternalOpenWindowAlarm | 外部传感器检测到开窗。  |

#### FrostProtectionAlarmEnum8 类型

该类型的底层类型为 `enum8`，用于表示防冻告警状态。

| 值    | 名称                  | 说明                       |
|-------|-----------------------|----------------------------|
| 0x00  | NoAlarm               | 无防冻告警。               |
| 0x01  | FrostProtectionAlarm  | 防冻保护处于激活状态。     |

#### ThermostatFeatureMap32 类型

`ThermostatFeatureMap32` 定义恒温器功能位图。该类型的底层类型为 map32。

| 位 | 掩码 | 名称 | 说明 |
|:---:|:---|:---|:---|
| 0 | 0x00000001 | Heating | Server 支持制热。 |
| 1 | 0x00000002 | Cooling | Server 支持制冷。 |

本 Profile 属性表与命令表中的 Feature 必选性使用上表 **名称**。

#### AqaraThermostatFeatureMap32 类型

`AqaraThermostatFeatureMap32` 定义 Aqara Thermostat 功能位图。该类型的底层类型为 map32。

| 位 | 掩码 | 名称 | 说明 |
|:---:|:---|:---|:---|
| 0 | 0x00000001 | OpenWindowDetection | Server 支持开窗检测与告警。 |
| 1 | 0x00000002 | FrostProtection | Server 支持防冻保护告警与可配置触发温度。 |
...
| 0x1001 | OccupiedHeatingSetpoint      | int16                            | 见描述        | —      | R/W/RP | S | Heating | 设定制热温度 |
| 0x1002 | LocalTemperature             | int16                            | 0.01℃         | —      | R/RP   | S | M | 环境温度
| 0x1003 | LocalTemperatureCalibration  | int8                             | 0.1℃          | 0      | R/W/RP | S | O | 本地温度校准
| 0x1004 | LocalConfigDisabled          | [LocalConfigDisabledEnum8](#localconfigdisabledenum8-类型)  | 见枚举        | 0x00   | R/W/RP | S | O | 本地配置禁用
| 0x1005 | TemperatureDisplayMode       | [TemperatureDisplayModeEnum8](#temperaturedisplaymodeenum8-类型) | 见枚举      | 0x00   | R/W/RP | S | O | 温度显示单位
| 0x1006 | FrostProtectionEnabled       | bool                             | true / false  | false  | R/W/RP | S | `[FrostProtection]` | 防冻使能
| 0x1007 | FrostProtectionTemperature   | int16                            | Min ~ Max，0.01℃ | 500 | R/W/RP | S | FrostProtection | 防冻触发温度
| 0x1008 | WindowDetectionEnabled       | bool                             | true / false  | false  | R/W/RP | S | OpenWindowDetection | 开窗检测使能
| 0x1009 | ActiveCurrent                | uint32                           | mA            | —      | RP     | S | O | 当前电流
| 0x100A | ActivePower                  | uint32                           | mW            | —      | RP     | S | O | 当前功率
| 0x100B | OpenWindowAlarm              | [OpenWindowAlarmEnum8](#openwindowalarmenum8-类型) | 见枚举       | 0x00 | R/RP | S | OpenWindowDetection | 开窗告警状态
| 0x100C | OpenWindowTemperatureControlPauseDuration | uint16              | Min ~ Max，min | 30 | R/W/RP | S | `[OpenWindowDetection]` | 开窗后温控暂停时长
| 0x100D | OpenWindowTemperatureControlPauseDurationMin | uint16           | min | 1 | R | S | `[OpenWindowDetection]` | 暂停时长下限
| 0x100E | OpenWindowTemperatureControlPauseDurationMax | uint16           | min | 1440 | R | S | `[OpenWindowDetection]` | 暂停时长上限
| 0x100F | FrostProtectionAlarm         | [FrostProtectionAlarmEnum8](#frostprotectionalarmenum8-类型) | 见枚举 | 0x00 | R/RP | S | FrostProtection | 防冻告警状态
| 0x1010 | FrostProtectionTemperatureMin | int16                           | 0.01℃ | 500 | R | S | FrostProtection | 防冻温度下限
| 0x1011 | FrostProtectionTemperatureMax | int16                           | 0.01℃ | 1500 | R | S | FrostProtection | 防冻温度上限
| 0x1012 | CumulativeEnergy             | int64                            | mWh           | —      | RP     | S | O | 累计输入电能
| 0x1014 | ThermostatFeatureMap         | [ThermostatFeatureMap32](#thermostatfeaturemap32-类型)    | Heating=0x1，Cooling=0x2 | 0x00000001 | R     | S | M | 恒温器功能位图
| 0x1015 | AbsMinHeatSetpointLimit      | int16                            | 0.01℃         | 500    | R      | S | `[Heating]` | 制热设定绝对下限
| 0x1016 | AbsMaxHeatSetpointLimit      | int16                            | 0.01℃         | 3000   | R      | S | `[Heating]` | 制热设定绝对上限
| 0x1017 | MinHeatSetpointLimit         | int16                            | 0.01℃         | 500    | R/W/RP | S | `[Heating]` | 制热设定用户下限
| 0x1018 | MaxHeatSetpointLimit         | int16                            | 0.01℃         | 3000   | R/W/RP | S | `[Heating]` | 制热设定用户上限
| 0x1019 | AqaraThermostatFeatureMap    | [AqaraThermostatFeatureMap32](#aqarathermostatfeaturemap32-类型) | OpenWindowDetection=0x1，FrostProtection=0x2 | 0x00000003 | R | S | O | Aqara 恒温器功能位图
| 0xFFFB | ServerAttributeList          | list[uint16]                    | 无额外约束    | — | R    | S | M | Server 属性列表
...
`OccupiedHeatingSetpoint` 表示有人占用场景下的制热目标温度，单位 0.01℃。例如 `2650` 表示 26.50℃，`-900` 表示 -9.00℃。当 `ThermostatFeatureMap` 的 Heating 位置位时，Server SHALL 实现该属性；未置位时 SHALL NOT 实现。

该属性已实现时，取值由以下已实现的限值共同决定。未实现的绝对限值分别以其表内默认值 `500` / `3000` 代替；未实现的用户上下限分别以对应的绝对限值（或其默认值）代替。

* `AbsMinHeatSetpointLimit` / `AbsMaxHeatSetpointLimit` 是制造商声明的制热设定绝对上下限。
* `MinHeatSetpointLimit` / `MaxHeatSetpointLimit` 是用户可配置的制热设定上下限。
* 用户上下限与绝对上下限均已实现时，SHALL 满足：
+
`AbsMinHeatSetpointLimit` ≤ `MinHeatSetpointLimit` ≤ `MaxHeatSetpointLimit` ≤ `AbsMaxHeatSetpointLimit`
* `OccupiedHeatingSetpoint` SHALL 满足有效下限 ≤ `OccupiedHeatingSetpoint` ≤ 有效上限，其中有效下限为已实现的 `MinHeatSetpointLimit`，否则为已实现的 `AbsMinHeatSetpointLimit`，再否则为默认值 `500`；有效上限为已实现的 `MaxHeatSetpointLimit`，否则为已实现的 `AbsMaxHeatSetpointLimit`，再否则为默认值 `3000`。

全部限值均已实现时，完整关系为：

`AbsMinHeatSetpointLimit` ≤ `MinHeatSetpointLimit` ≤ `OccupiedHeatingSetpoint` ≤ `MaxHeatSetpointLimit` ≤ `AbsMaxHeatSetpointLimit`

该属性已实现时，Server SHALL 在目标温度变化后 REPORT 该属性。
...
`LocalTemperatureCalibration` 表示本地温度校准偏移，单位 0.1℃。例如 `20` 表示 +2.0℃。Server MAY 实现该属性。

该属性已实现时，最终本地温度 = `LocalTemperature` + `LocalTemperatureCalibration`，Client MAY WRITE 该属性调整校准偏移。未实现该属性时，不得叠加校准偏移，等效于校准值为 `0`。

#### LocalConfigDisabled 属性

LocalConfigDisabled` 表示是否禁用本地配置。Client MAY WRITE 该属性。

有效值见 [LocalConfigDisabledEnum8](#localconfigdisabledenum8-类型)。
...
`FrostProtectionEnabled` 表示防冻保护是否启用。`true` 表示启用，`false` 表示禁用。

当该属性为 `true` 时，Server SHALL 根据 `FrostProtectionTemperature` 监测用于防冻判断的实测温度。当该属性为 `false` 时，Server SHALL NOT 应用防冻保护行为，并 MAY 将 `FrostProtectionAlarm` 设置为 `NoAlarm`。未实现该属性时，是否启用防冻保护由产品定义。

#### FrostProtectionTemperature 属性

`FrostProtectionTemperature` 表示防冻保护触发温度，单位 0.01℃。该属性的值 SHALL 大于等于 `FrostProtectionTemperatureMin`，且小于等于 `FrostProtectionTemperatureMax`。

当 `FrostProtectionEnabled` 存在且为 `true`，并且用于防冻判断的实测温度低于该值时，Server SHALL 触发产品定义的防冻行为并将 `FrostProtectionAlarm` 设置为 `FrostProtectionAlarm`。若 `FrostProtectionEnabled` 不存在，防冻触发行为由产品定义。

#### WindowDetectionEnabled 属性

`WindowDetectionEnabled` 表示开窗检测功能是否启用。`true` 表示启用，`false` 表示禁用。

当该属性为 `true` 时，Server MAY 使用快速温度变化、外部传感器输入或产品定义的其他逻辑检测开窗。当该属性为 `false` 时，Server SHALL NOT 应用开窗检测专有的温控行为，并 MAY 将 `OpenWindowAlarm` 设置为 `NoAlarm`、立即恢复温控。
...
`OpenWindowAlarm` 表示当前开窗告警状态，有效值见 [OpenWindowAlarmEnum8](#openwindowalarmenum8-类型)。

检测到开窗且 `WindowDetectionEnabled` 为 `true` 时，Server **应** 将该属性设置为 `InternalOpenWindowAlarm` 或 `ExternalOpenWindowAlarm`，并进入控制暂停。告警清除后，Server **应** 将该属性设置为 `NoAlarm` 并恢复温控。

`OpenWindowAlarm` 的值发生变化时，Server **应** REPORT 该属性。`ClearOpenWindowAlarmReq` 命令执行成功时，Server **应** 清除告警并立即恢复温控。

#### OpenWindowTemperatureControlPauseDuration 属性

`OpenWindowTemperatureControlPauseDuration` 表示开窗告警触发后控制暂停的持续时间，单位为分钟。该属性的值 **应** 大于等于 `OpenWindowTemperatureControlPauseDurationMin`，且小于等于 `OpenWindowTemperatureControlPauseDurationMax`。

“控制暂停”是指 Server 暂停为维持当前目标温度而产生的主动制热或制冷输出，例如停止驱动阀门、继电器、压缩机或其他温控执行器。控制暂停不等同于关闭设备，也不改变 `SystemMode`、目标温度及其他用户配置。暂停期间，Server **应** 继续执行温度测量、属性读写与上报、命令处理及产品定义的安全保护；安全保护行为 **优先于** 控制暂停。

开窗告警触发后，Server **应** 进入控制暂停并保持该属性指定的时长。暂停时间到期后，Server **应** 根据当前 `SystemMode`、目标温度和实测温度恢复正常温控。若告警在计时结束前被清除，Server **应** 立即结束控制暂停并重新进行温控判断。

#### OpenWindowTemperatureControlPauseDurationMin 属性

`OpenWindowTemperatureControlPauseDurationMin` 表示 `OpenWindowTemperatureControlPauseDuration` 支持的最小值，单位为分钟。该属性由 Server 声明设备能力。Client SHALL 在 Profile Discovery 完成后 READ。该属性仅用于能力声明，Server SHALL NOT REPORT。

#### OpenWindowTemperatureControlPauseDurationMax 属性

`OpenWindowTemperatureControlPauseDurationMax` 表示 `OpenWindowTemperatureControlPauseDuration` 支持的最大值，单位为分钟。该属性由 Server 声明设备能力。Client SHALL 在 Profile Discovery 完成后 READ。该属性仅用于能力声明，Server SHALL NOT REPORT。

#### FrostProtectionAlarm 属性

`FrostProtectionAlarm` 表示当前防冻告警状态，有效值见 [FrostProtectionAlarmEnum8](#frostprotectionalarmenum8-类型)。防冻保护激活时，Server **应** 将该属性设置为 `FrostProtectionAlarm`；防冻保护退出后，Server **应** 将该属性设置为 `NoAlarm`。

该属性的值发生变化时，Server **应** REPORT。

#### FrostProtectionTemperatureMin 属性

`FrostProtectionTemperatureMin` 表示 `FrostProtectionTemperature` 支持的最小值，单位为 0.01℃。该属性由 Server 声明设备能力。Client SHALL 在 Profile Discovery 完成后 READ。该属性仅用于能力声明，Server SHALL NOT REPORT。

#### FrostProtectionTemperatureMax 属性

`FrostProtectionTemperatureMax` 表示 `FrostProtectionTemperature` 支持的最大值，单位为 0.01℃。该属性由 Server 声明设备能力。Client SHALL 在 Profile Discovery 完成后 READ。该属性仅用于能力声明，Server SHALL NOT REPORT。

#### CumulativeEnergy 属性

`CumulativeEnergy` 表示累计输入电能，单位 mWh，底层类型为 int64。该值 SHALL 单调不减。Server SHOULD 按产品定义的周期或变化策略 REPORT 该属性。

#### ThermostatFeatureMap 属性

`ThermostatFeatureMap` 表示恒温器功能位图。有效值见 [ThermostatFeatureMap32](#thermostatfeaturemap32-类型)。该值由 Server 声明。

Client SHALL 在 Profile Discovery 完成后 READ 该属性。该属性仅用于能力声明，不存在运行期变化上报场景，Server SHALL NOT REPORT。

#### AbsMinHeatSetpointLimit 属性

`AbsMinHeatSetpointLimit` 表示制热设定的绝对下限，单位 0.01℃。该属性由 Server 声明。

若该属性出现在 `ServerAttributeList` 中，Client SHALL 在 Profile Discovery 完成后 READ 该属性。该属性仅用于能力声明，不存在运行期变化上报场景，Server SHALL NOT REPORT。与 `OccupiedHeatingSetpoint` 及其他制热限值的关系见 [OccupiedHeatingSetpoint 属性](#occupiedheatingsetpoint-属性)。

#### AbsMaxHeatSetpointLimit 属性

`AbsMaxHeatSetpointLimit` 表示制热设定的绝对上限，单位 0.01℃。该属性由 Server 声明。

若该属性出现在 `ServerAttributeList` 中，Client SHALL 在 Profile Discovery 完成后 READ 该属性。该属性仅用于能力声明，不存在运行期变化上报场景，Server SHALL NOT REPORT。与 `OccupiedHeatingSetpoint` 及其他制热限值的关系见 [OccupiedHeatingSetpoint 属性](#occupiedheatingsetpoint-属性)。

#### MinHeatSetpointLimit 属性

`MinHeatSetpointLimit` 表示用户可设置的制热设定下限，单位 0.01℃。若 `AbsMinHeatSetpointLimit` 已实现，该值 SHALL 大于等于 `AbsMinHeatSetpointLimit`；若 `MaxHeatSetpointLimit` 已实现，该值 SHALL 小于等于 `MaxHeatSetpointLimit`。Client MAY WRITE 该属性。完整关系见 [OccupiedHeatingSetpoint 属性](#occupiedheatingsetpoint-属性)。

#### MaxHeatSetpointLimit 属性

`MaxHeatSetpointLimit` 表示用户可设置的制热设定上限，单位 0.01℃。若 `AbsMaxHeatSetpointLimit` 已实现，该值 SHALL 小于等于 `AbsMaxHeatSetpointLimit`；若 `MinHeatSetpointLimit` 已实现，该值 SHALL 大于等于 `MinHeatSetpointLimit`。Client MAY WRITE 该属性。完整关系见 [OccupiedHeatingSetpoint 属性](#occupiedheatingsetpoint-属性)。

#### AqaraThermostatFeatureMap 属性

`AqaraThermostatFeatureMap` 表示 Aqara Thermostat 功能位图。有效值见 [AqaraThermostatFeatureMap32](#aqarathermostatfeaturemap32-类型)。该值由 Server 声明。

若该属性出现在 `ServerAttributeList` 中，Client SHALL 在 Profile Discovery 完成后 READ 该属性。该属性仅用于能力声明，不存在运行期变化上报场景，Server SHALL NOT REPORT。
...
| ID      | 名称                      | 方向               | 应答                   | 必选性 | 简述                       |
|---------|---------------------------|--------------------|------------------------|--------|----------------------------|
| 0x1000 | ClearOpenWindowAlarmReq | Client → Server | ClearOpenWindowAlarmRsp | OpenWindowDetection | 清除当前开窗告警并立即恢复温控。 |
| 0x1001 | ClearOpenWindowAlarmRsp | Server → Client | N | OpenWindowDetection | 返回清除操作结果。|

有参命令的参数定义见各命令 **参数** 表；Data Segment 编码规则见 [Command](#commandopc--0x9)。

#### ClearOpenWindowAlarmReq 命令

该命令请求 Server 清除激活的开窗告警。命令执行成功后，Server **应** 将 `OpenWindowAlarm` 设置为 `NoAlarm`，并立即恢复温控，不受剩余 `OpenWindowTemperatureControlPauseDuration` 限制。

该命令无参数。Server **应** 使用配对命令 `ClearOpenWindowAlarmRsp` 返回执行结果，不再另行返回 ACKNOWLEDGE。

#### ClearOpenWindowAlarmRsp 命令

该命令为 `ClearOpenWindowAlarmReq` 的配对返回命令。Client 收到后 **应** 不再返回 ACKNOWLEDGE。
...
`Success` 表示命令的业务执行结果：`true` 表示操作成功，`false` 表示操作失败。
...
- `OtaChunkData` 复用 `OtaChunkRequest` 的 `Seq`（本例均为 `0x06`）。
- Server 收到 `OtaChunkData` 后不再返回 ACKNOWLEDGE。
