---
title: "红外设备接入"
description: "本文描述如何通过 Studio 本地 API 将红外设备接入 Aqara Studio，从而通过网关下发红外指令控制红外设备。"
...
| RemoteId    | integer  | ✔       | 红外码库 ID，即 [3.3 获取匹配树](#33-获取匹配树) 接口返回的 `controller_id`。         |
| KeyId       | string   | ✔       | 红外指令 ID，即 [3.3 获取匹配树](#33-获取匹配树) 接口返回的 `nodes.key.ac_key`（有状态空调） 或 `nodes.key.id`（其他设备）。 |
