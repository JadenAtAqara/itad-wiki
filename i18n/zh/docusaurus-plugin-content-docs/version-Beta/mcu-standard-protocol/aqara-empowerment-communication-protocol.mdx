...
| Wake Signaling（唤醒信令） | 发送方在 UART 通信前，通过对端 Wake Pin 执行低电平断言、T0 等待、UART 传输与引脚释放的完整过程。参见 [第 4 章](#第四章-初始化)。
| Link Establishment（链路建立） | 上电后首个 AECP 交互过程；Client 发送 `LinkEstablishmentRequest`，Server 返回 `LinkEstablishmentResponse(LowPowerFeature)`。参见 [第 4 章](#第四章-初始化)。
| Profile Discovery（Profile 发现） | Client READ Server Profile 与属性初始值的过程；完成后 Client 发送 `ProfileDiscoveryComplete`。参见 [第 4 章](#第四章-初始化)。
| LowPowerFeature（低功耗特性） | `LinkEstablishmentResponse` 携带的单一参数，用于协商 Wake Signaling 是否使能；**双方协商完成前默认使能**；`Normal` 表示协商关闭。参见 [第 4 章](#第四章-初始化)。
...
| `Octets` / `Raw Data Octets`        | COMMAND Payload Format 中每个参数 Raw Data 的字节数（不含前置 `Data Type` 字节）。变长字段 SHALL 写明长度计算方式。
| `Data Type`                         | COMMAND Payload Format 中每个参数的类型标识；在 Data Segment 中 SHALL 以 1 字节 `Data Type` 前缀出现在对应 Raw Data 之前。 |
| `Field Name`                        | Wire Layout 表格中 Raw Data 列的语义名称。 |
| `Wire Layout`                       | 某条 COMMAND 的完整 Data Segment 字节序：表格从左到右即线上顺序，含 Cmd ID 与各 `Data Type`、Raw Data 列。 |
## 第二章 硬件接口
...
- **唤醒引脚（Wake Pin）**：用于低功耗设备的双向唤醒信号（Wake Signaling 使用该引脚，参见 [第四章](#第四章-初始化)）。Client 和 Server 各自提供一个唤醒引脚，分别用于被对方唤醒。
...
每个参数 SHALL 按 `Data Type (1 B) + Raw Data (N B)` 编码，Raw Data 格式与 WRITE 属性块中的 Data 字段一致（不含 Attribute ID）。参数 SHALL 按对应 Profile 中该命令 `Parameter Details` 表的 `ID` 从小到大依次排列；不携带参数数量字段。

- **无参数命令**：D-Len = `2`，Data Segment 仅含 Cmd ID。
- **有参数命令**：D-Len = `2 + Σ(1 + sizeof(raw_i))`。
- 参数名称、顺序、类型、长度和约束由 Profile 命令定义中的 **Wire Layout** 决定。
...
1. **链路建立**：Client 发送 `LinkEstablishmentRequest`，Server 返回 `LinkEstablishmentResponse(LowPowerFeature)`，协商 Wake Signaling 是否使能。双方协商完成前，Wake Signaling 特性**默认使能**；协商完成后，仅当 Server 返回 `Normal` 时不使能。
...
Link Establishment 是上电后的首个 AECP 交互过程，用于协商 Wake Signaling 是否使能（`LowPowerFeature`）。协商完成前，Wake Signaling 特性默认使能。

**完成判据：** Client 收到 `LinkEstablishmentResponse` 并缓存 `LowPowerFeature`。

**交互过程：**

- Client **SHALL** 发送 `LinkEstablishmentRequest`（无参数）作为上电首条 AECP 帧；发送前 **SHALL** 执行 Wake Signaling（协商前默认使能）。
- Server **SHALL** 返回 `LinkEstablishmentResponse`，携带单一参数 `LowPowerFeature`（`LowPowerFeatureEnum8`，参见第六章相关内容）；发送前 **SHALL** 执行 Wake Signaling。


:::tip
协商完成前：Wake Signaling 特性默认使能，协商交互期间，发送方 **SHALL** 执行 Wake Signaling。
:::
...
`LowPowerFeature` 是 `LinkEstablishmentResponse` 携带的单一参数，类型为 `LowPowerFeatureEnum8`，用于协商 Wake Signaling 是否使能。  
...
| 阶段 | 名称 | 说明 |
|:---:|:---|:---|
| 阶段 1 | 链路建立 | 协商 `LowPowerFeature`；协商前 Wake Signaling 默认使能；协商交互发送方 **SHALL** Wake Signaling；`Normal` 协商结果不使能 |
...
Client 上电后 **SHALL** 首先执行链路建立：

1. Client 发送 `LinkEstablishmentRequest` 前 **SHALL** 执行 Wake Signaling（协商前默认使能）。
2. Server 返回 `LinkEstablishmentResponse(LowPowerFeature)` 前 **SHALL** 执行 Wake Signaling。
3. Client 缓存 `LowPowerFeature`；链路建立协商完成。协商前 Wake Signaling 默认使能；仅当协商结果为 `Normal` 时不使能。
...
* Client **SHALL** 发送 `LinkEstablishmentRequest` 作为上电首条 AECP 交互，协商 `LowPowerFeature`；发送前 **SHALL** 执行 Wake Signaling（协商前默认使能）。
* Server **SHALL** 返回 `LinkEstablishmentResponse(LowPowerFeature)`；发送前 **SHALL** 执行 Wake Signaling。
* 双方协商 `LowPowerFeature` 完成前，Wake Signaling 特性默认使能；协商交互期间，发送方 **SHALL** 执行 Wake Signaling。
...
| 0x0009 | ConnectionDisableRequest | Server → Client | ACKNOWLEDGE           | O           | Server 请求 Client 关闭无线连接。 |
| 0x000A | ProfileDiscoveryRequest        | Server → Client | ACKNOWLEDGE           | M           | Server 请求 Client 重新执行 Profile Discovery。 |
| 0x000B | LinkEstablishmentRequest    | Client → Server | LinkEstablishmentResponse | M       | Client 发起链路建立。 |
| 0x000C | LinkEstablishmentResponse   | Server → Client | N                     | M           | Server 返回低功耗特性。 |
...
#### Command Payload 编码约定

带参 COMMAND 的 **Data Segment** 即 COMMAND 帧中 OPC 之后的载荷，自左向右按下列字节序编码：
...
- **Cmd ID**：2 B 大端无符号整数，取值见上表 `ID` 列。
- **参数块**：每个参数由 1 B `Data Type` 与紧随其后的 `Raw Data` 组成；`Raw Data` 编码规则与 WRITE 属性块中的 Data 相同（参见 [数据类型总览](#数据类型总览)）。
- **参数顺序**：各命令下方 **Wire Layout** 表格从左到右即线上字节顺序；第 1 个参数块紧跟 Cmd ID，其后参数依次排列。
- **D-Len**：Data Segment 总字节数，等于 Wire Layout 各列长度之和（参见 [Command](#commandopc--0x9)）。无参命令 `D-Len = 2`；有参命令 `D-Len = 2 + Σ(1 + RawDataLen_i)`。
- 下文 **Wire Layout** 给出完整字段名与长度；**字段说明** 给出语义与约束。

#### LinkEstablishmentRequest 命令

该命令由 Client 在上电后作为首条 AECP 交互发送，用于发起 Link Establishment（参见 [LinkEstablishmentRequest 命令](#linkestablishmentrequest-命令)）。发送前 Client **SHALL** 执行 Wake Signaling（协商前默认使能）。

该命令无参数。

#### LinkEstablishmentResponse 命令

该命令由 Server 响应 `LinkEstablishmentRequest` 发送，用于返回 Server 低功耗特性。该命令为配对返回命令，Client 收到后不再返回 ACKNOWLEDGE。
...
| -- | -- | -- |
| 2 B | 1 B | 1 B |
| `0x000C` | `0x30` (enum8) | enum8 值 |

Data Segment 共 4 B。

##### 字段说明

`LowPowerFeature` 声明 Server 低功耗特性，用于协商 Wake Signaling 是否使能。有效值见 [LowPowerFeatureEnum8](#lowpowerfeatureenum8-类型)。**双方协商完成前，Wake Signaling 特性默认使能**；Server 返回 `Normal` 表示协商关闭 Wake Signaling（参见 [第四章](#第四章-初始化)）。

Server 报告的 `LowPowerFeature` **SHALL** 与产品实际低功耗能力一致。未实现低功耗 sleep 与 Wake Pin 机制的产品 **SHALL** 返回 `Normal`。

#### ProfileDiscoveryComplete 命令

该命令由 Client 在完成 Profile Discovery（参见 [第四章](#第四章-初始化)）后发送，用于通知 Server：Client 侧 Profile Discovery 已经完成，Server 可以开始主动数据同步。
...
该命令由 Server 发送，用于请求 Client 重新执行 Profile Discovery（参见 [第四章](#第四章-初始化)）。典型场景包括 Server 能力变更、Profile 或 `ServerAttributeList` 更新后，需要 Client 重新读取 Server 属性并同步能力视图。
...
#### ConnectionDisableRequest 命令

该命令由 Server 发送，用于请求 Client 关闭无线连接。Client 接受后 **必须（SHALL）** 关闭射频收发功能并断开无线网络连接，并通过 ACKNOWLEDGE 返回执行结果。

Client 关闭无线连接后，**建议（SHOULD）** 将 `NetworkStatus` 更新为 `Offline`，并在状态变化时通过 NOTIFY 进行同步。
...
|:---|:---:|:---:|:---:|:---:|
| 2 B | 1 B | 4 B | 1 B | 4 B |
| `0x0020` | `0x23` (uint32) | uint32 大端 | `0x23` (uint32) | uint32 大端 |
...
| -- | -- | -- | -- | -- |
| 2 B | 1 B | 4 B | 1 B | 1 B |
| `0x0022` | `0x23` (uint32) | uint32 大端 | `0x20` (uint8) | uint8 |
...
| `0x0023` | `0x23` (uint32) | uint32 大端 | `0x41` (octstr) | Length 前缀 + 分片字节 |

Data Segment 共 `9 + Length` B（`Length` 为 octstr 分片长度，1 ~ 255）。
...
| -- | -- | -- |
| 2 B | 1 B | 1 B
| `0x0026` | `0x30` (enum8) | [OtaAbortReasonEnum8](#otaabortreasonenum8-类型) 值 |
