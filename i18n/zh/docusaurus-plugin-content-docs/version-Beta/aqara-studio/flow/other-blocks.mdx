...
| 连接点/参数        | 是否必选 | 说明 |
|-------------------|:--:|------|
| 向上计数（输入点） | ✘ | 连接 [当](./../flow/device-blocks.mdx#当) 或 [且](./device-blocks.mdx#且) 卡片，每当对应事件发生时，计数值递增。 |
| 向下计数（输入点） | ✘ | 连接 [当](./../flow/device-blocks.mdx#当) 或 [且](./device-blocks.mdx#且) 卡片，每当对应事件发生时，计数值递减。 |
| 预设（输入点）     | ✘ | 连接 [当](./../flow/device-blocks.mdx#当) 或 [且](./device-blocks.mdx#且) 卡片，每当对应事件发生时，计数值被重置为“预设值”。 |
| 清除（输入点）     | ✘ | 连接 [当](./../flow/device-blocks.mdx#当) 或 [且](./device-blocks.mdx#且) 卡片，每当对应事件发生时，计数值归零。 |
| 预设值（参数）     | ✘ | 设定计数器在“预设”事件发生时要重置到的目标值。 |
| 计数增量（参数）   | ✔ | 每次计数的增减幅度，默认为 1，可自定义。|
| 输出点            | ✔ | 输出当前计数值，连接至可接收数值的卡片。 |