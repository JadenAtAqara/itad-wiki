...
1. 在首页点击进入**通信**页面。
2. 在侧边栏单击“+”按钮，在弹窗中选择 `Modbus TCP`，以添加 `Modbus TCP` 文件夹。
3. 右键单击 `Modbus TCP` 文件夹，选择**新增**，此时你可以看到该目录下出现一个 `Modbus TCP 设备` 文件夹。
...
8. 单击**类型选择**区域中的**+**按钮，新增测点。单击该点，即可设置其类型和点位配置。完成后，单击**确定**。
   ...
   <table>
     ...
     <tbody>
       ...
       <tr>
         ...
         <td>布尔型点必填</td>
         <td>
           <p>当点位为<strong>布尔型</strong>时选择，支持以下：</p>
           <ul>
...
但由于 Modbus 报文中使用的是从 0 开始的内部地址，而用户看到的地址通常是从 1、30001、40001 等逻辑地址开始的，所以 Aqara Studio Connect 需要一个“偏移地址”来进行自动转换。

例如，某个测点的真实地址是 40001（保持寄存器）。如果你在设置 Modbus TCP 设备时将“保持寄存器偏移地址”设置为 40000，那么，在为这个设备添加点时，你只需要填写数据地址为 1，Aqara Studio Connect 就会自动加上偏移量，计算为：
