...
| dataType  | 功能点数据类型，支持以下取值：<ul><li><code>bool</code> / <code>boolean</code>：布尔型</li><li><code>numerical</code> / <code>number</code>：数值型</li><li><code>enum</code>：枚举型</li><li><code>text</code> / <code>string</code>：文本型</li></ul> |
| enumRange | 枚举选项数组。仅当通用功能点的 `dataType` 为 `enum` 时必填。数组不能为空；每项包含整数类型的 `value` 和字符串类型的 `label`，`value` 和 `label` 均不可重复，且 `label` 不可为空。 |
| min | 数值型通用功能点的最小值。仅当 `dataType` 为 `numerical` / `number` 时生效；也可使用 `minimum` 作为别名。 |
| max | 数值型通用功能点的最大值。仅当 `dataType` 为 `numerical` / `number` 时生效；也可使用 `maximum` 作为别名。 |
| step | 数值型通用功能点的步长，必须大于 0。仅当 `dataType` 为 `numerical` / `number` 时生效；也可使用 `resolution` 作为别名。 |
| precision | 数值型通用功能点的精度值，必须大于等于 0。仅当 `dataType` 为 `numerical` / `number` 时生效。 |
| decimals | 数值型通用功能点的小数位数，必须为大于等于 0 的整数。仅当 `dataType` 为 `numerical` / `number` 时生效。 |
| unit / units | 数值型通用功能点的单位。仅当 `dataType` 为 `numerical` / `number` 时生效；可填写系统已注册的单位名称或符号，例如 `kWh` / `kilowatt hour`、`W` / `watt`、`V` / `volt`、`A` / `ampere`。 |
...
如果数值点会被能耗统计或其他依赖单位的功能使用，请配置 `unit` 或 `units`，例如累计电量使用 `kWh`，当前功率使用 `W`，电压使用 `V`，电流使用 `A`。单位必须是系统已注册的单位名称或符号。
:::
