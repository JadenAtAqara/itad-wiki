...
下图展示了协议事件进入 JavaScript 后的通用处理过程。每个解析器会复用自己的 Context；Context 被回收后，下一次调用会重新创建 Context 并重新加载完整脚本。

```mermaid
flowchart TD
    A["脚本加载或 HTTP 业务事件"] --> B{"输入大小和脚本长度符合限制？"}
    B -- "否" --> R["拒绝本次调用并记录原因"]
    B -- "是" --> C{"执行队列、内存保护和超时熔断允许执行？"}
    C -- "否" --> R
    C -- "是" --> D["进入绑定的串行执行队列"]
    D --> E{"Context 是否存在？"}
    E -- "否" --> F["创建 Context<br/>注入 console 和 studioScriptState<br/>重新加载完整脚本"]
    E -- "是" --> G["复用现有 Context"]
    F --> H["调用当前业务函数"]
    G --> H
    H --> I{"函数执行和返回结构是否有效？"}
    I -- "否" --> J["忽略无效数据或将本次业务标记为失败"]
    I -- "是" --> K["发送 HTTP 请求<br/>创建设备或功能点<br/>更新功能点值"]
    H -- "执行超时或 Context 失效" --> N["关闭 Context 并短时熔断"]
    J --> L["限频检查 studioScriptState 大小"]
    K --> L
    L --> M["返回业务结果"]
    N --> O["后续允许执行时按需重建 Context"]
```
