...
| data | Object | 是 | 请求数据。 |
| data.types | Array of String | 否 | 设备类型列表。如需了解可用字符串，请参考 [设备类型说明](./device-type.mdx)。 |
...
| data.data[].types | Array of String | 设备类型。字符串含义请参考 [设备类型说明](./device-type.mdx)。 |
...
| data[].types | Array of String | 设备类型列表。字符串含义请参考 [设备类型说明](./device-type.mdx)。 |
...
- 响应中的 `data[].types`，表示设备支持的类型列表，请参考 [设备类型](./device-type.mdx) 了解字符串相关含义。
...
- 设备添加。
- 设备删除。
- （仅适用于非 Aqara 设备）设备增加了端点。
- （仅适用于非 Aqara 设备）设备增加了端点。
- （仅适用于非 Aqara 设备）设备功能点的属性（最小值、最大值、单位等）更新。
...
| data | Object | 事件数据对象。 |
| data.eventType | String | 事件类型：<ul><li>`DEVICE_ADDED`：设备添加</li><li>`DEVICE_REMOVED`：设备删除。</li><li>`DEVICE_ENDPOINT_ADDED`：（仅适用于非 Aqara 设备）设备增加了端点。</li><li>`DEVICE_ENDPOINT_REMOVED`：（仅适用于非 Aqara 设备）设备删除了端点。</li><li>`DEVICE_TRAIT_ATTRIBUTE_UPDATE`：（仅适用于非 Aqara 设备）设备功能点的属性（最小值、最大值、单位等）更新。</li></ul> |
...
##### 非 DEVICE_TRAIT_ATTRIBUTE_UPDATE 事件
...
| name | String | 设备名称。 |
| types | Array of String | 设备类型列表。字符串含义请参考 [设备类型说明](./device-type.mdx)。 |
| endpointIds | Array of Number | 端点 ID 列表。 |

##### DEVICE_TRAIT_ATTRIBUTE_UPDATE 事件

:::说明

若事件为 `DEVICE_TRAIT_ATTRIBUTE_UPDATE`，建议重新调用 `GetDevicesRequest` 接口 [查询设备配置](#查询设备的-spec-配置)。
...
```json
{
    ...
    "msgId": "1765163175639",
    "data": {
        "eventType": "DEVICE_TRAIT_ATTRIBUTE_UPDATE",
        "objectId": "xxxxxxx",
        "time": 1765163175603,
        "data": {
