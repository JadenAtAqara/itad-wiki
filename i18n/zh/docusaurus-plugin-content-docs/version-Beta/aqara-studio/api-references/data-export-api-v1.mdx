...
:::tip
`code` 为 `0` 仅表示接口调用成功，并不意味着设备状态已改变。请通过 [查询功能点的最新值](./data-export-api-v1.mdx#查询设备的最新功能点值) 确认是否已成功控制设备。
::: 
...
若事件为 `DEVICE_TRAIT_PARAMETER_UPDATE`，建议重新调用 `GetDevicesRequest` 接口 [查询设备配置](#查询设备的-spec-配置)。
