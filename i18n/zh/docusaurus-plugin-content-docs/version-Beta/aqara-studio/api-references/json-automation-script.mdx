...
| metadata.name        | String           | ✗      | 自动化的名称（仅供开发者参考，不向用户显示）。    |
...
| automations.condition   | Object     | ✗        | 执行条件，详情情参考 [Condition 说明](#condition)。                 |
...
| 类型           | 说明                             |
| -------------- | -------------------------------- |
| [PropertyEvent](#propertyevent)  | 当设备属性变化并满足特定条件时触发。 |
| [TimeSchedule](#timeschedule)   | 在指定的时间或按 Cron 表达式定时触发。 |
...
| is                  | [Dynamic](#比较运算符的类型说明)    | ✗       | 检查事件数据是否等于某个值。                                    |
| isNot               | [Dynamic](#比较运算符的类型说明)    | ✗       | 检查事件数据是否不等于某个值。                                  |
| greaterThan         | [Dynamic](#比较运算符的类型说明)    | ✗       | 检查事件数据是否大于某个值。                               |
| greaterThanOrEqualTo| [Dynamic](#比较运算符的类型说明)    | ✗       | 检查事件数据是否大于等于某个值。                           |
| lessThan            | [Dynamic](#比较运算符的类型说明)    | ✗       | 检查事件数据是否小于某个值。                                |
| lessThanOrEqualTo   | [Dynamic](#比较运算符的类型说明)    | ✗       | 检查事件数据是否小于等于某个值。                           |
| for                 | [Duration](#duration)   | ✗       | 条件需要持续满足的时间（如 "30min"、"1hour10min20sec"）。           |
| suppressFor         | [Duration](#duration)   | ✗       | 触发后抑制再次触发的时间。                                      |
...
- 字面量值：基本数值类型，如 Boolean、Number、String。
...
- AM/PM 格式：
    ...
    | h:mm:ss am/pm   | 8:00:00 am    | 早上 8 点整（秒可选）    |
...
- `property.event.for`：触发器条件需要持续满足的时间。
- `property.event.suppressFor`：触发后抑制再次触发的时间。
