...
4. 在已创建的订阅中，将 Aqara Studio 提供的 **请求 Topic 格式**中的 ClientID 替换为 MQTTX 自动生成的实际 ClientID，并填写到 Topic 输入框。参照 [数据转出 API](./api-references/data-export-api-v1.mdx) 文档中各接口的请求参数说明（忽略 Header 部分），编写 JSON 格式的消息并发送，即可获得响应。

    下图以 [查询设备 ID 列表](./api-references/data-export-api-v1.mdx#查询设备-id-列表) 的请求为例：
