...
下图展示了 TCP/UDP 报文进入 JavaScript 后的通用处理过程。每个解析器会复用自己的 Context；Context 被回收后，下一次调用会重新创建 Context 并重新加载完整脚本。

```mermaid
flowchart TD
    A["脚本加载或 TCP/UDP 完整报文"] --> B{"报文大小和脚本长度符合限制？"}
    B -- "否" --> R["拒绝本次调用并记录原因"]
    ...
    E -- "是" --> G["复用现有 Context"]
    F --> H["调用 dispatch(bytes)"]
    G --> H
    H --> I{"dispatch 返回了什么？"}
    I -- "path" --> J["调用 path 指定的解析函数"]
    I -- "仅 msgId" --> K["交给请求事务按 msgId 匹配"]
    I -- "path 和 msgId 都为空" --> L["按主动上报调用 parseReportRequest"]
    J --> M{"返回结构、数量、点类型和值有效？"}
    L --> M
    M -- "否" --> N["忽略无效数据或将本次业务标记为失败"]
    M -- "是" --> O["更新设备或功能点状态<br/>按需生成响应报文"]
    H -- "执行超时或 Context 失效" --> P["关闭 Context 并短时熔断"]
    N --> Q["限频检查 studioScriptState 大小"]
    O --> Q
    K --> Q
    Q --> S["返回业务结果"]
    P --> T["后续允许执行时按需重建 Context"]
```

各 TCP/UDP 业务节点还会执行以下校验：
