...
| min | Number | 数值型通用点位的最小值。仅当 `dataType` 为 `numerical` 时生效；也可使用 `minimum` 作为别名 |
| max | Number | 数值型通用点位的最大值。仅当 `dataType` 为 `numerical` 时生效；也可使用 `maximum` 作为别名 |
| step | Number | 数值型通用点位的步长，必须大于 0。仅当 `dataType` 为 `numerical` 时生效；也可使用 `resolution` 作为别名 |
| precision | Number | 数值型通用点位的精度值，必须大于等于 0。仅当 `dataType` 为 `numerical` 时生效 |
| decimals | Int | 数值型通用点位的小数位数，必须为大于等于 0 的整数。仅当 `dataType` 为 `numerical` 时生效 |
| unit / units | String | 数值型通用点位的单位。仅当 `dataType` 为 `numerical` 时生效；可填写系统已注册的单位名称或符号，例如 `kWh` / `kilowatt hour`、`W` / `watt`、`V` / `volt`、`A` / `ampere` |
